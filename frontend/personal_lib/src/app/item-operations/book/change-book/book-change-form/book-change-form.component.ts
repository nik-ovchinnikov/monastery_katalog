...
export class BookChangeFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.oldPictures = this.bookService.itemToChange.pictureList;
  ...
  onSubmit() {
    ...
    for(let picture of this.bookService.newPictures) {
      this.bookService.changedItem.pictureList.push(picture);
    }
  ...
  onFileAdded(event: any) {
          if(event.target.nextSibling.getAttribute("wasUploaded") == "false"){
            ...
            this.bookService.newPictures.push(new Picture());
          }else{
            //удаляем старую фото
            let fileToRemove: File =  new File([], "");
  ...
  onClickDeleteOldPicture(event: any){
    ...
    this.bookService.changedItem.pictureList.splice(index, 1);
  }
  ...
  resetChanges() {
    ...
    this.myForm.patchValue({
      ...
      typeName: this.bookService.itemToChange.type.name,
      subTypeName: this.bookService.itemToChange.subType.name,
      placeName: this.bookService.itemToChange.place.name,
      subPlaceName: this.bookService.itemToChange.subPlace.name,
    });
    ...
    for(let picture of this.bookService.oldPicturesToDelete) {
      this.bookService.itemToChange.pictureList.push(picture);
  ...
  cleanForm() {
        this.bookService.selectedFiles = [];
        this.bookService.changedItem.pictureList = [];
        this.bookService.newPictures = [];
        this.bookService.oldPicturesToDelete = [];
        ...
        this.bookService.changedItem.subPlace = new SubPlace();
        this.bookService.changedItem.subType= new SubType();
  ...
  formInitialization() {
    this.myForm = this.fb.group({
      ...
      description: [this.bookService.itemToChange.description],
      typeName: [this.bookService.itemToChange.type.name, Validators.required],
      subTypeName: [this.bookService.itemToChange.subType.name, Validators.required],
      placeName: [this.bookService.itemToChange.place.name, Validators.required],
      subPlaceName: [this.bookService.itemToChange.subPlace.name, Validators.required],
  ...
  formChangeSubscribe() {
    ...
    this.myForm.get("typeName")?.valueChanges.subscribe((data) => {
      ...
      this.bookService.changedItem.type =this.chosenType;
    ...
    this.myForm.get("placeName")?.valueChanges.subscribe((data) => {
      ...
      this.bookService.changedItem.place = this.chosenPlace;
    ...
    this.myForm.get("subTypeName")?.valueChanges.subscribe((data) => {
      ...
      this.bookService.changedItem.subType = this.chosenSubType;
    });
    ...
    this.myForm.get("subPlaceName")?.valueChanges.subscribe((data) => {
      ...
      this.bookService.changedItem.subPlace = this.chosenSubPlace;
    });
  ...
  createItemChanged() {
    ...
    this.bookService.changedItem.place = Object.assign(this.bookService.itemToChange.place);
    this.bookService.changedItem.subPlace = Object.assign(this.bookService.itemToChange.subPlace);
    this.bookService.changedItem.type = Object.assign(this.bookService.itemToChange.type);
    this.bookService.changedItem.subType = Object.assign(this.bookService.itemToChange.subType);
    this.bookService.changedItem.author = this.bookService.itemToChange.author;
    this.bookService.changedItem.publishYear = this.bookService.itemToChange.publishYear;
    this.bookService.changedItem.publisher = this.bookService.itemToChange.publisher;
    this.bookService.changedItem.seria = this.bookService.itemToChange.seria;
    this.bookService.changedItem.weight = this.bookService.itemToChange.weight;
    this.bookService.changedItem.size = this.bookService.itemToChange.size;

  }

  prepareChosenVars() {
    this.chosenPlace = this.bookService.itemToChange.place;
    this.chosenType = this.bookService.itemToChange.type
    this.chosenSubType = this.bookService.itemToChange.subType;
    this.chosenSubPlace = this.bookService.itemToChange.subPlace;
  }
