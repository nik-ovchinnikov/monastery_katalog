...
@Component({
  selector: 'app-add-icon-form',
  templateUrl: './add-icon-form.component.html',
  styleUrls: ['./add-icon-form.component.css']
})
export class AddIconFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.typeService.urlParam = 'icon';
    this.placeService.urlParam = 'icon';
    ...
    this.myForm.get("author")?.valueChanges.subscribe((data) => {
      this.itemService.resultItem.author= data;
    });
