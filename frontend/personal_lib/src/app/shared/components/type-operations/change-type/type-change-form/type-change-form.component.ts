...
export class TypeChangeFormComponent implements OnInit, OnDestroy {
  ...
  //из компонента добавление типа
    ...
    for(let subType of this.typeService.typeToChange.subTypeList){
  ...
  onSubmit() {
    ...
    this.typeService.changeType(this.typeService.typeToChange.subTypeList);
...
//
  ...
  removeSubType(i: number) {
    ...
    let subtypes: SubType[] = this.typeService.typeToChange.subTypeList;
  ...
  subTypeArrayRendering(subTypeArr: any) {
    ...
    for(let st of subTypeArr) {
      if (index < this.typeService.typeToChange.subTypeList.length){
        let currentSubType = this.typeService.typeToChange.subTypeList[index]
        currentSubType.name = st['subTypeName'];
        currentSubType.description = st['subTypeDescription'];
        currentSubType.lastChangeDate = new Date();
        currentSubType.typeName = this.myForm.value.name;
      } else {
        this.typeService.typeToChange.subTypeList.push(new SubType(
          st.subTypeName,
  ...
  resetChanges() {
    ...
    for(let st of this.typeService.deletedSubtypes) {
      this.typeService.typeToChange.subTypeList.push(st);
    }
    //удалить из массива удалённыъ подтипов
