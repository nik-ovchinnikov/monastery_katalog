...
export class PlaceChangeFormComponent implements OnInit, OnDestroy {
  ...
  //из компонента добавление типа
    ...
    for(let subPlace of this.placeService.placeToChange.subPlaceList){
  ...
  onSubmit() {
    ...
    this.placeService.changePlace(this.placeService.placeToChange.subPlaceList);
...
//
  ...
  removeSubPlace(i: number) {
    ...
    let subplaces: SubPlace[] = this.placeService.placeToChange.subPlaceList;
  ...
  subPlaceArrayRendering(subPlaceArr: any) {
    ...
    for(let sp of subPlaceArr) {
      if (index < this.placeService.placeToChange.subPlaceList.length){
        let currentSubPlace = this.placeService.placeToChange.subPlaceList[index]
        ...
        currentSubPlace.placeName = this.myForm.value.name;
      } else {
        this.placeService.placeToChange.subPlaceList.push(new SubPlace(
          sp.subPlaceName,
  ...
  resetChanges() {
    ...
    for(let sp of this.placeService.deletedSubplaces) {
      this.placeService.placeToChange.subPlaceList.push(sp);
    }
    //удалить из массива удалённыъ подтипов
