...
//        logger.info("Изменена книга: " + book.toString());
        ...
        System.out.println(book.getPictureList());
...
//                }
    ...
    public void deleteItemList(Book[] books) {
        for(Book book: books) {
            System.out.println(book);
            for(BookPicture picture: book.getPictureList()) {
    ...
    public void changeItemsIfDeleteSubType(Long id) {
       ...
       List<Book> bookList = this.bookDao.findBySubType(bookSubType);
    ...
    public void changeItemsIfDeleteType(Long id) {
        ...
        for(BookSubType subType: bookType.getSubTypeList()){
            changeItemsIfDeleteSubType(subType.getId());
        }
        List<Book> bookList = this.bookDao.findByType(bookType);
        for(Book book : bookList) {
            book.setType(null);
            bookDao.save(book);
        }
...
//        System.out.println(bookPlace);
        for(BookSubPlace subPlace: bookPlace.getSubPlaceList()){
            changeItemsIfDeleteSubPlace(subPlace.getId());
        }
        List<Book> bookList = this.bookDao.findByPlace(bookPlace);
...
//        System.out.println(bookList);
    ...
    public void changeItemsIfDeleteSubPlace(Long id) {
        ...
        List<Book> bookList = this.bookDao.findBySubPlace(bookSubPlace);
        for(Book book : bookList) {
